...
BIN_PATH="$BIN_DIR/nova"
CODE_MODE_HOST_BIN_PATH="$BIN_DIR/nova-code-mode-host"
NOVA_HOME_DIR="${NOVA_HOME:-$HOME/.nova}"
STANDALONE_ROOT="$NOVA_HOME_DIR/packages/standalone"
...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-alpha(\.[0-9]+){0,2}|-beta(\.[0-9]+)?)?$'; then
    echo "Invalid Nova release version: $version. Expected latest or x.y.z[-alpha[.N[.M]]|-beta[.N]]." >&2
    return 1
  fi
}
...
download_file() {
  ...
  echo "curl or wget is required to install Nova." >&2
  exit 1
}
...
release_metadata_url() {
  ...
  printf 'https://api.github.com/repos/xuanlinAI/nova-code/releases/tags/rust-v%s\n' "$resolved_version"
}
...
parse_downloaded_release_metadata() {
  ...
  if ! release_metadata="$(printf '%s\n' "$release_json" | parse_release_metadata)"; then
    echo "Could not parse $source_name release metadata for Nova $requested_release." >&2
    return 1
  fi
}
...
resolve_metadata_version() {
  ...
  if [ -z "$metadata_version" ]; then
    echo "Failed to resolve the latest Nova release version." >&2
    return 1
  fi
...
resolve_release_from_github() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    metadata_url="https://api.github.com/repos/xuanlinAI/nova-code/releases/latest"
  else
  ...
  if ! release_json="$(download_text "$metadata_url")"; then
    echo "Could not fetch GitHub release metadata for Nova $requested_release. GitHub API may be unavailable or rate limited." >&2
    exit 1
  fi
...
select_release_assets() {
  package_asset="nova-package-$vendor_target.tar.gz"
  checksum_asset="nova-package_SHA256SUMS"
  ...
  if release_asset_exists "$package_asset" &&
    ...
    asset="$package_asset"
  elif release_asset_exists "nova-npm-$npm_tag-$resolved_version.tgz"; then
    install_layout="legacy-platform-npm"
    asset="nova-npm-$npm_tag-$resolved_version.tgz"
  else
    echo "Could not find Nova package or platform npm release assets for Nova $resolved_version." >&2
    return 1
  fi
...
package_archive_digest() {
  ...
  if [ -z "$digest" ]; then
    echo "Could not find SHA-256 digest for $asset in nova-package_SHA256SUMS." >&2
    return 1
  fi
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Nova download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Nova archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Nova." >&2
    exit 1
  fi
}
...
add_to_path() {
  ...
  begin_marker="# >>> Nova installer >>>"
  end_marker="# <<< Nova installer <<<"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.nova.*' -exec rm -f {} +
  fi
}
...
version_from_binary() {
  ...
  "$nova_path" --version 2>/dev/null | sed -n 's/.* \([0-9][0-9A-Za-z.+-]*\)$/\1/p' | head -n 1
}

current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/nova" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi

  version="$(version_from_binary "$CURRENT_LINK/nova" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_nova() {
  command -v nova 2>/dev/null || true
}

classify_existing_nova() {
  existing_path="$1"
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && nova"
      step "Future terminals: open a new terminal and run: nova"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: nova"
      step "Future terminals: open a new terminal and run: nova"
      ;;
  esac
}

maybe_launch_nova_now() {
  if prompt_yes_no "Start Nova now?"; then
    step "Launching Nova"
    "$BIN_PATH"
  fi
}
...
detect_conflicting_install() {
  existing_path="$(resolve_existing_nova)"
  manager="$(classify_existing_nova "$existing_path" || true)"
  ...
  step "Detected existing $manager-managed Nova at $existing_path"
  warn "Multiple managed Nova installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Nova now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Nova. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Nova installed. PATH order will determine which nova runs."
  fi
}
...
install_package_release() {
  ...
  chmod 0755 \
    "$stage_release/bin/nova" \
    "$stage_release/bin/nova-code-mode-host" \
    "$stage_release/nova-path/rg"
  if [ -f "$stage_release/nova-resources/bwrap" ]; then
    chmod 0755 "$stage_release/nova-resources/bwrap"
  fi
  ln -sf "bin/nova" "$stage_release/nova"
...
install_legacy_platform_npm_release() {
  ...
  mkdir -p "$stage_release/nova-resources" "$extract_dir"
  tar -xzf "$archive_path" -C "$extract_dir"

  cp "$vendor_root/nova/nova" "$stage_release/nova"
  cp "$vendor_root/path/rg" "$stage_release/nova-resources/rg"
  chmod 0755 "$stage_release/nova" "$stage_release/nova-resources/rg"
  if [ -f "$vendor_root/nova-resources/bwrap" ]; then
    cp "$vendor_root/nova-resources/bwrap" "$stage_release/nova-resources/bwrap"
    chmod 0755 "$stage_release/nova-resources/bwrap"
  fi
...
release_dir_is_complete() {
  ...
  case "$layout" in
    package)
      [ -f "$release_dir/nova-package.json" ] &&
        [ -x "$release_dir/bin/nova" ] &&
        [ -x "$release_dir/bin/nova-code-mode-host" ] &&
        [ -x "$release_dir/nova" ] &&
        [ -x "$release_dir/nova-path/rg" ] ||
        return 1
      ;;
    legacy-platform-npm)
      [ -x "$release_dir/nova" ] &&
        [ -x "$release_dir/nova-resources/rg" ] ||
        return 1
      ;;
    *)
      return 1
  ...
  case "$layout:$expected_target" in
    package:*linux* | legacy-platform-npm:*linux*)
      [ -x "$release_dir/nova-resources/bwrap" ] || return 1
      ;;
  esac

  installed_version="$(version_from_binary "$release_dir/bin/nova" || version_from_binary "$release_dir/nova" || true)"
...
release_nova_relative_path() {
  release_dir="$1"

  if [ -x "$release_dir/bin/nova" ]; then
    printf 'bin/nova\n'
  else
    printf 'nova\n'
  fi
...
update_visible_command() {
  release_dir="$1"
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.nova.$$"
  nova_relative_path="$(release_nova_relative_path "$release_dir")"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/$nova_relative_path" "$tmp_link"

  if [ "$os" = "darwin" ] && [ -x "$release_dir/bin/nova-code-mode-host" ]; then
    replace_path_with_symlink \
      "$CODE_MODE_HOST_BIN_PATH" \
      "$CURRENT_LINK/bin/nova-code-mode-host" \
      "$tmp_link"
  elif [ "$(readlink "$CODE_MODE_HOST_BIN_PATH" 2>/dev/null || true)" = \
    "$CURRENT_LINK/bin/nova-code-mode-host" ]; then
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Nova CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Nova CLI"
else
  step "Installing Nova CLI"
fi
...
if ! release_dir_is_complete "$release_dir" "$resolved_version" "$vendor_target" "$install_layout"; then
  echo "Installed Nova command did not report expected version $resolved_version." >&2
  exit 1
fi
...
printf 'Nova CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_nova_now
